...
import { MessageUsageTrendResponse } from "@/lib/services/analytic/types";
import { ErrorResponse, SuccessResponse } from "@/lib/services/responseType";
import { NextRequest, NextResponse } from "next/server";

export async function GET(request: NextRequest){
    try{
        const res = await analyticService.getMessageTrend(request)
        ...
        return NextResponse.json<SuccessResponse<MessageUsageTrendResponse[]>>(data, {status: res.status})
    }catch (error: any){

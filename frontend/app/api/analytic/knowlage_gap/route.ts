...
import { KnowledgeGapResponse } from "@/lib/services/analytic/types";
import { ErrorResponse, SuccessResponse } from "@/lib/services/responseType";
import { NextRequest, NextResponse } from "next/server";

export async function GET(request: NextRequest){
    try{
        const res = await analyticService.getKnowledgeGap(request)
        ...
        return NextResponse.json<SuccessResponse<KnowledgeGapResponse>>(data, {status: res.status})
    }catch (error: any){
